...
# structlog se importa normalmente.
# chromadb se importa de forma LAZY dentro de __init__ para evitar crash
# en Python 3.12+ donde opentelemetry tiene incompatibilidades con chromadb.
import structlog
...
class RAGEngine:
    ...
    def __init__(
        ...
        # Import lazy de chromadb — si falla, RAGEngine igual puede importarse
        # pero las funciones de búsqueda lanzarán RuntimeError descriptivo.
        try:
            import chromadb
    ...
    def _require_chroma(self):
        """Lanza error descriptivo si ChromaDB no está disponible."""
        if self.chroma_client is None:
            raise RuntimeError(
                "ChromaDB no está disponible en este entorno. "
                "Verifica que chromadb esté instalado y sea compatible "
                "con la versión de Python en uso."
            )
    ...
    def index_chunks(
        ...
        try:
            ...
            logger.info("chunks_indexed", collection=collection_name, count=len(documents))
    ...
    def retrieve(
        ...
        """Recupera chunks relevantes de ChromaDB."""
        collection = self.get_or_create_collection(collection_name)
        try:
    ...
    def query_stream(
        ...
        """Búsqueda RAG con streaming — para uso con st.write_stream()."""
        ...
        yield from self.llm.generate_stream(
            prompt=prompt, system=_RAG_SYSTEM, max_tokens=max_tokens,
        )
        yield (
            "\n\n---\n"
        ...
        logger.info("rag_stream_completed", question=question[:100], chunks=len(sources))
    ...
    def get_collection_stats(self, collection_name: str = "documents") -> dict:
        ...
        try:
            collection = self.get_or_create_collection(collection_name)
            return {"nombre": collection_name, "documentos_indexados": collection.count()}
        except Exception:
